...
void cp(char *filename_old, char *filename_new)
{
	...
	if (fd_old == -1)
	{
		dprintf(STDER_FILENO, "Error: Can't read from file %s", filename_old);
		exit(98);
	}
	fd_new = open(filename_new, O_CREAT | O_TRUNC | O_RDWR, 0664);
	if (fd_new == -1)
	{
		dprintf(STDER_FILENO, "Error: Can't write to %s", filename_new);
		exit(99);
	}
	...
	if (c_old == -1)
	{
		dprintf(STDER_FILENO, "Error: Can't close fd %d", fd_old);
		exit(100);
	}
	c_new = close(fd_new);
	if (c_new == -1)
	{
		dprintf(STDER_FILENO, "Error: Can't close fd %d", fd_new);
		exit(100);
	}
}
/**
